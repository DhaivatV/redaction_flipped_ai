...
def redact(cv_path, redaction_list, link_text, name_text, short_links, dname):
    ...
    for page in doc:
        ...
        for s_link in short_links:
            ...
            for area in areas:
                [page.add_redact_annot(area, fill=(0, 0, 0)) for area in areas]
                page.apply_redactions()
    doc.save(f'20th_july_redacted2/{dname}', garbage=3, deflate=True)
...
def cvredact(sname, dname):
    Data = extract_text_from_pdf(sname)
    urls = []
    ...
    redact(sname, redactions, links_text, name_text, llink_text, dname)
    rdoc = remove_img_on_pdf(dname)
    rdoc.saveIncr()
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("cv_redact.csv")
    uuids = df['redacted'].to_list()
    cvs = df['original'].to_list()

    for item in res:

        id = ''.join(random.choices(string.ascii_letters + string.digits, k=18))
        fname = id + '.pdf'
        while fname in uuids:
            id = ''.join(random.choices(string.ascii_letters + string.digits, k=18))
            fname = id + '.pdf'
        ...
        try:
            cvredact(item, fname)
            uuids.append(fname)
            cvs.append(item.split('/')[:-1])
        except Exception as e:
            uuids.append('NA')
            cvs.append(item.split('/')[:-1])
    df['original'] = pd.Series(cvs)
    df['redacted'] = pd.Series(uuids)
    df.to_csv('cv_redacted.csv', index=False)